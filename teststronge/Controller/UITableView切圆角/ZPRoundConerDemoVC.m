...
#import "ZPRoundConerTableViewCell.h"
//#import "ZBNetworking.h"

@interface ZPRoundConerDemoVC ()<UITableViewDataSource,UITableViewDelegate>
{
    // cell圆角
...
@implementation ZPRoundConerDemoVC
...
//        config.baseURL=@"";//如果同一个环境，有多个域名 不建议设置baseURL
...
//    NSData *data = UIImageJPEGRepresentation([UIImage imageNamed:@"sign_bg"], 0.5);;
//    NSString *BaseUrl= @"http://care-api.zjzhishu.com/api/v1";
//    NSString *uploadUrl = @"/upload/index";
//    [ZBRequestManager requestWithConfig:^(ZBURLRequest * _Nonnull request) {
//        request.URLString = [NSString stringWithFormat:@"%@%@",BaseUrl,uploadUrl];
//        request.headers = @{@"userid":@"QXhj4bMRVYhW2L2bXw1hSw=="};
...
//        [request addFormDataWithName:@"files" fileName:fileName mimeType:@"image/jpg/png/jpeg" fileData:data];
...
- (NSArray *)numberOfRowForSections {
    if (!_numberOfRowForSections) {
        self.numberOfRowForSections = @[@10,@50,@18,@40];
    }
