...
#import "YLWaterFlowLayout_Bug.h"
#import "NSArray+YLBoundsCheck.h"

@interface YLWaterFlowLayout_Bug()
@property(nonatomic,strong)NSMutableArray *framesArray;
@end


@implementation YLWaterFlowLayout_Bug
#pragma mark - 初始化属性
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    for(NSInteger i = 0; i < tmpArray.count; i++){
        ...
        if(category == UICollectionElementCategoryCell){//如果是cell
            [array addObject:[self layoutAttributesForItemAtIndexPath:attrs.indexPath]];
        }else if (category == UICollectionElementCategorySupplementaryView){//如果是supplementView (section头 尾)
...
//返回对应于indexPath的位置的追加视图的布局(section header footer)属性，如果没有追加视图可不重载
